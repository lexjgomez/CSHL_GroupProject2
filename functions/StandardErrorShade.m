function [curve] = StandardErrorShade(amatrix,alpha,acolor,linestyle,F,smth)
% usage: stdshading(amatrix,alpha,acolor,F,smth)
% plot mean and sem/std coming from a matrix of data, at which each row is an
% observation. sem/std is shown as shading.
% - acolor defines the used color (default is red) 
% - linestyle the linestyle of plotting 
...
astd = nanstd(amatrix,[],1)/sqrt(size(amatrix, 1)); % to get standard error shading
...
hold on;curve = plot(F,amean,'color', acolor,'linewidth',1.5,'linestyle',linestyle); %% change color or linewidth to adjust mean line
if check
    hold off;
end
end
